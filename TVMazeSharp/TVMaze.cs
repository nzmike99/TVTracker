    ...
    public class TVMaze
    {
        ...
        private static async Task<T> _Get<T>(Uri lookup)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                {
                    results = responseBodyAsText.Replace("\n", " ");
                    results = results.Replace("\\n", " ");
                    results = results.Replace("<br>", Environment.NewLine);
                ...
                try
                {
                    return JsonConvert.DeserializeObject<T>(_404Stub);
                }
                catch (JsonSerializationException)
                {
                    return default(T);
        ...
        public static async Task<IReadOnlyCollection<SearchContainerShow>> FindSeries(string Query)
        {
            ...
            if (string.IsNullOrWhiteSpace(Query))
                return new SearchContainerShow[0];
        ...
        public static async Task<IReadOnlyCollection<SearchContainerPerson>> FindPerson(string Query)
        {
            Query = Uri.UnescapeDataString(Query);
            if (string.IsNullOrWhiteSpace(Query))
                return new SearchContainerPerson[0];
        ...
        private static async Task<MazeSeries> _GetSeries(Uri uri, bool FetchEpisodes = false, bool FetchCast = false)
        {
            ...
            {
                if (FetchEpisodes)
                    await series.getEpisodes();
                if (FetchCast)
                    await series.getCast();
            }
            return series;
        }
        ...
        /// <param name="FetchEpisodes">Fetch episodes in the show, or leave null.</param>
        /// <param name="FetchCast">Fetch Cast in the show, or leave null.</param>
        public static async Task<MazeSeries> FindSingleSeries(string Query, bool FetchEpisodes = false, bool FetchCast = false)
        {
            Query = Uri.EscapeDataString(Query);
            return await _GetSeries(new Uri(SID_LOOKUP_SINGLE + Query), FetchEpisodes, FetchCast);
        }
        /// <summary>
        /// Search the scaper by TVMaze Series ID.
        /// </summary>
        ...
        /// <param name="FetchEpisodes">Fetch episodes in the show, or leave null.</param>
        /// <param name="FetchCast">Fetch Cast in the show, or leave null.</param>
        public static async Task<MazeSeries> GetSeries(uint TVMazeID, bool FetchEpisodes = false, bool FetchCast = false)
        {
            return await _GetSeries(new Uri(SERIES_LOOKUP + TVMazeID), FetchEpisodes, FetchCast);
        }
        /// <summary>
        /// Search the scaper by TheTVDB Series ID.
        /// </summary>
        ...
        /// <param name="FetchEpisodes">Fetch episodes in the show, or leave null.</param>
        /// <param name="FetchCast">Fetch Cast in the show, or leave null.</param>
        public static async Task<MazeSeries> GetSeriesfromTVDBID(uint TVDBID, bool FetchEpisodes = false, bool FetchCast = false)
        {
            return await _GetSeries(new Uri(SERIES_LOOKUP_TVDB + TVDBID), FetchEpisodes, FetchCast);
        }
        /// <summary>
        /// Search the scaper by TVRage Series ID.
        /// </summary>
        ...
        /// <param name="FetchEpisodes">Fetch episodes in the show, or leave null.</param>
        /// <param name="FetchCast">Fetch Cast in the show, or leave null.</param>
        public static async Task<MazeSeries> GetSeriesfromTVRageID(uint TVRageID, bool FetchEpisodes = false, bool FetchCast = false)
        {
            return await _GetSeries(new Uri(SERIES_LOOKUP_TVRAGE + TVRageID), FetchEpisodes, FetchCast);
        }
        ...
        public static async Task<List<MazeEpisode>> GetSeries_Episodes(uint TVMazeID)
        {
           // Task<List<MazeEpisode>> eps = _Get<List<MazeEpisode>>(new Uri(SERIES_LOOKUP + TVMazeID + SERIES_EPISODES));

           // eps.sel
            // MazeEpisode[] episodes = _Get<MazeEpisode[]>(new Uri(SERIES_LOOKUP + TVMazeID + SERIES_EPISODES));
            return await _Get<List<MazeEpisode>>(new Uri(SERIES_LOOKUP + TVMazeID + SERIES_EPISODES));
        }
        
        //USE THIS VERSION IF EPISODES ARE TO BE A WRITEABLE COLLECTION
        //public static async Task<ObservableCollection<MazeEpisode>> GetSeries_Episodes(uint TVMazeID)
    //{
    //    //We need the episodes in an observable collection as we want to add a watched flag and be able to toggle it.
    //    //IReadOnlyCollection<MazeEpisode> roEps = await _Get<MazeEpisode[]>(new Uri(SERIES_LOOKUP + TVMazeID + SERIES_EPISODES));
    //    ObservableCollection<MazeEpisode> eps = new ObservableCollection<MazeEpisode>(
    //        await _Get<MazeEpisode[]>(new Uri(SERIES_LOOKUP + TVMazeID + SERIES_EPISODES)));
    //    return eps;    //await _Get<MazeEpisode[]>(new Uri(SERIES_LOOKUP + TVMazeID + SERIES_EPISODES));
